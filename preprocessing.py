...
def _load_data(imagepath, ptspath, is_train,mirror_array):
    ...
    if is_train:
        pts = pts - center
        pts = np.dot(pts, makerotate(np.random.normal(0, 20))) #旋转
        pts = pts * np.random.normal(0.8, 0.05)#缩放
        pts = pts + [np.random.normal(0, 0.05),#平移
    ...
    #print("save pts",pts,filename)
    return img,pts.astype(np.float32)

def _input_fn(img, pts, is_train,mirror_array):
    dataset_image = tf.data.Dataset.from_tensor_slices(img)#创建dataset
    ...
    for f1 in fs:
        ...
        if not os.path.isdir(tmp_path):
            ...
            if ext == "jpg" or ext == "png" or ext == "bmp":
                #print('文件: %s'%tmp_path)
                imageset.extend(glob.glob(tmp_path))
        else:
    ...
    with tf.Session() as sess:
        ...
        while True:
            try:
                ...
                imgs_mean = np.mean(img_list,axis=0)#计算矩阵均值
                imgs_std = np.std(img_list,axis=0)#计算矩阵标准差
