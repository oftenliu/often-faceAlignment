...
def add_weight_decay(weight_decay,scope):
    ...
    kernels = [v for v in trainable_vars if scope in v.name and 'kernel'  in v.name]
    for K in kernels:
        print("weight name",K.name)
...
def dan_model_fn(features,
    ...
    if 0:#use wing loss 
        loss_s1 = wing_loss(resultdict['s1_ret'], groundtruth, w=10, epsilon=2.0)
        loss_s2 = wing_loss(resultdict['s2_ret'], groundtruth, w=10, epsilon=2.0)
    else:
    ...
    if stage == 1:
        tf.losses.add_loss(loss_s1)
        add_weight_decay(5e-4,"s1")
        regularization_loss = tf.losses.get_regularization_loss()
        loss_s1 = tf.losses.get_total_loss(add_regularization_losses=True)
    else:
        tf.losses.add_loss(loss_s2)
        add_weight_decay(5e-4,"s2")
        regularization_loss = tf.losses.get_regularization_loss()
        loss_s2 = tf.losses.get_total_loss(add_regularization_losses=True)


    #loss_s1 = tf.reduce_mean(tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.squared_difference(groundtruth,resultdict['s1_ret']),-1)),-1) / tf.sqrt(tf.reduce_sum(tf.squared_difference(tf.reduce_max(groundtruth,1),tf.reduce_min(groundtruth,1)),-1)))
    #loss_s2 = tf.reduce_mean(tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.squared_difference(groundtruth,resultdict['s2_ret']),-1)),-1) / tf.sqrt(tf.reduce_sum(tf.squared_difference(tf.reduce_max(groundtruth,1),tf.reduce_min(groundtruth,1)),-1)))

    #add evaluate code by lqc 2019 2.11
    if mode == tf.estimator.ModeKeys.EVAL:
        eval_metric_ops = {
            'validation_mae': tf.metrics.mean_absolute_error(groundtruth, resultdict['s2_ret']),
            'normalized_mean_error': nme_metric_ops(groundtruth, resultdict['s2_ret'])
        }
        return tf.estimator.EstimatorSpec(
            mode, predictions=resultdict,loss=loss_s2,
...
def dan_main(flags, model_function, input_function):
    ...
    def input_fn_eval():
        return input_function(False, flags.data_dir if flags.data_dir_test is  None else flags.data_dir_test, flags.batch_size,
